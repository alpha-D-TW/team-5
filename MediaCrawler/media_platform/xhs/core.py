...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    async def search(self) -> None:
        ...
        out_size = 0
        for k in config.KEYWORDS.split(","):
            keyword = config.MAIN + k ;
            ...
            while page * xhs_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                out_size += len(note_details)
                utils.logger.info(f"[XiaoHongShuCrawler.search] cur note details size: {len(note_details)}, total size: {out_size}")
    ...
    async def get_comments(self, note_id: str, semaphore: asyncio.Semaphore):
        ...
        async with semaphore:
            ...
            # 更新或保存过滤后的评论
            await xhs_store.batch_update_xhs_note_comments(note_id=note_id, comments=filtered_comments)
